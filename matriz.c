...
void multMatVetOtimizado (MatRow mat, Vetor v, int m, int n, Vetor res) {
  if (res) {
    for (int ii = 0; ii < (m + BK - 1) / BK; ++ii) {
      int istart = ii * BK;
      int iend = MIN(istart + BK, m);
      for (int i = istart; i < iend; i += UR) {
        for (int j = 0; j < m; ++j) {
          int unroll_end = MIN(i + UR, iend);
          for (int k = i; k < unroll_end; ++k) {
            res[k] += mat[k * m + j] * v[j];
          }
        }
      }
...
void multMatMatOtimizado (MatRow A, MatRow B, int n, MatRow C) {
  for (int ii = 0; ii < (n + BK -1) / BK; ++ii) {
    int istart = ii * BK;
    int iend = MIN(istart + BK, n);
    for (int jj = 0; jj < (n + BK - 1) / BK; ++jj) {
      int jstart = jj * BK;
      int jend = MIN(jstart + BK, n);
      for (int kk = 0; kk < (n + BK - 1) / BK; ++kk) {
        int kstart = kk * BK;
        int kend = MIN(kstart + BK, n);

        for (int i = istart; i < iend; i += UR) {
          for (int j = jstart; j < jend; ++j) {
            for (int k = kstart; k < kend; ++k) {
              int unroll_end = MIN(i + UR, iend);
              for (int u = i; u < unroll_end; ++u) {
                C[u * n + j] += A[u * n + k] * B[k * n + j];
              }
            }
          }
        }
